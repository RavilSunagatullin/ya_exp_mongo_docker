...
export const fillUsersStateFromPage = (userId) => {
  ...
  setCurrentUserState('username', targetElementsState.find((item) => item.name === 'username').element.textContent)
  setCurrentUserState('email', targetElementsState.find((item) => item.name === 'email').element.textContent)
}
...
const setButtonStyle = (userId, state) => {
  if (state) {
    document.querySelector(`#user-${userId} .edit-user-button svg use`).href.baseVal = '/svg/icons.svg#save'
    document.querySelector(`#user-${userId} .edit-user-button .visually-hidden`).textContent = 'Сохранить'
    document.querySelector(`#user-${userId} .edit-user-button svg`).style.stroke = 'lightgreen'
  } else {
    document.querySelector(`#user-${userId} .edit-user-button svg use`).href.baseVal = '/svg/icons.svg#edit'
    document.querySelector(`#user-${userId} .edit-user-button .visually-hidden`).textContent = 'Редактировать'
    document.querySelector(`#user-${userId} .edit-user-button svg`).removeAttribute('style')
  }
}
...
const setCloseButtonStyleAndListeners = (userId, state) => {
  if (state) {
    const handleCloseButtonClick = () => {
      ...
      document.querySelector(`#user-${userId} .close-item`).removeEventListener('click', handleCloseButtonClick)
      removePutUserListeners()
      addUsersEditModeListeners()
      document.querySelector(`#user-${userId} .close-item`).removeAttribute('style')
    }
    document.querySelector(`#user-${userId} .close-item`).style.display = 'block'
    document.querySelector(`#user-${userId} .close-item`).addEventListener('click', handleCloseButtonClick)
  }
}
