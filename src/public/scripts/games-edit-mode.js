...
const useEditableElementsState = (gameId) => {
  const targetElementsState = [
    ...
    {
      name: 'categoriesForm',
      element: [...document.querySelectorAll(`#game-${gameId} .game-categories-form`)],
      canEditText: false,
    ...
    {
      name: 'imageInput',
      element: [...document.querySelectorAll(`#game-${gameId} .image-url-input`)],
      canEditText: false,
      canSetTransparency: false,
      canSetVisibility: true,
      canSetSource: false,
...
export const fillStoreWithEditableElements = (gameId) => {
  ...
  targetElementsState.forEach((item) => {
    ...
    if (item.name === 'categoriesForm') {
      ...
      const categoriesIdArray = checkedCategories.map((category) => category.value)
      setCurrentState('categories', categoriesIdArray)
    }
  })
}
...
const setCloseButtonStyleAndListeners = (gameId, state) => {
  ...
  if (state) {
    ...
    document.querySelector(`#game-${gameId} .close-item`).removeAttribute('style')
  }
}
