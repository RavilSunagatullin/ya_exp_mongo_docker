...
export const fillCategoriesStateFromPage = (categoryId) => {
  ...
  setCurrentCategoryState('name', document.querySelector(`#category-${categoryId} .name`).textContent)
}
...
const setButtonStyle = (categoryId, state) => {
  if (state) {
    document.querySelector(`#category-${categoryId} .edit-category-button svg use`).href.baseVal = '/svg/icons.svg#save'
    document.querySelector(`#category-${categoryId} .edit-category-button .visually-hidden`).textContent = 'Сохранить'
    document.querySelector(`#category-${categoryId} .edit-category-button svg`).style.stroke = 'lightgreen'
  } else {
    document.querySelector(`#category-${categoryId} .edit-category-button svg use`).href.baseVal = '/svg/icons.svg#edit'
    document.querySelector(`#category-${categoryId} .edit-category-button .visually-hidden`).textContent =
      'Редактировать'
    document.querySelector(`#category-${categoryId} .edit-category-button svg`).removeAttribute('style')
  }
}
...
const setCloseButtonStyleAndListeners = (categoryId, state) => {
  if (state) {
    const handleCloseButtonClick = () => {
      ...
      document.querySelector(`#category-${categoryId} .close-item`).removeEventListener('click', handleCloseButtonClick)
      removePutCategoryListeners()
      addCategoriesEditModeListeners()
    }
    document.querySelector(`#category-${categoryId} .close-item`).style.display = 'block'
    document.querySelector(`#category-${categoryId} .close-item`).addEventListener('click', handleCloseButtonClick)
  } else {
    document.querySelector(`#category-${categoryId} .close-item`).removeAttribute('style')
  }
}
...
function handleButtonClick(event) {
  ...
  const categoryButtons = [...document.querySelectorAll('.edit-category-button')]
